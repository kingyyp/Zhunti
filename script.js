...
function playChimeSound() {
    ...
    frequencies.forEach((freq, index) => {
        ...
        osc.connect(oscGain); oscGain.connect(sourceGain);
    ...
    sourceGain.connect(masterGain); sourceGain.connect(delayNode);
...
function playSyntheticBg(type, isPreview = false) {
    ...
    if (type === 'muyu') {
        ...
        osc.type = 'sine'; osc.frequency.setValueAtTime(165, now); 
        oscSub.type = 'triangle'; oscSub.frequency.setValueAtTime(82.5, now); 
        osc.connect(gain); oscSub.connect(gain); gain.connect(audioCtx.destination);
        gain.gain.setValueAtTime(2.2, now); gain.gain.exponentialRampToValueAtTime(0.001, now + 0.25);
        osc.start(); oscSub.start(); osc.stop(now + 0.25); oscSub.stop(now + 0.25);
        ...
        baseFreqs.forEach((freq, i) => {
            ...
            osc.connect(sourceGain); osc.start(now); osc.stop(now + duration);
        });
        const filter = audioCtx.createBiquadFilter();
        filter.type = 'lowpass'; filter.frequency.setValueAtTime(150, now);
        const delayNode = audioCtx.createDelay(1.0);
        ...
        const masterGain = audioCtx.createGain();
        masterGain.connect(audioCtx.destination);
        sourceGain.connect(filter); filter.connect(masterGain); filter.connect(delayNode);  
        delayNode.connect(feedbackGain); feedbackGain.connect(delayNode); feedbackGain.connect(masterGain);
    }
}

bgMusicSelect.addEventListener('change', (e) => { if (e.target.value !== 'none') playSyntheticBg(e.target.value, true); });

function triggerBgSound() {
    const bgType = bgMusicSelect.value;
    if (bgType !== 'none' && isPlaying) playSyntheticBg(bgType, false);
}
...
function renderTotalStats() {
    ...
    progressBarText.textContent = rewardText !== "" ? `${rewardText} | 本階進度: ${pct.toFixed(2)}%` : `終身功德總進度: ${pct.toFixed(2)}% (距${currentMilestoneTarget.toLocaleString()}遍)`;
}
...
manualCountBtn.addEventListener('click', () => {
    playChimeSound(); count++;
    currentCountDisplay.textContent = count;
    addTotalCount(1); updateProgress(); saveLiveCountToToday(1);
    const alertTrigger = parseInt(alertIntervalSelect.value);
    if (alertTrigger > 0 && count % alertTrigger === 0) triggerToastPopup();
});
...
recordBtn.addEventListener('click', async () => {
    ...
    if (!isRecording) {
        ...
        try {
            ...
            if (!MediaRecorder.isTypeSupported('audio/mp4') && MediaRecorder.isTypeSupported('audio/webm')) options = { mimeType: 'audio/webm' };
        ...
        } catch (err) { alert("無法開啟麥克風，請檢查權限設定。"); }
    } else { mediaRecorder.stop(); isRecording = false; recordBtn.textContent = "🔴 重新錄製準提咒"; recordBtn.classList.remove('recording'); }
});
...
nativeAudio.addEventListener('ended', () => {
    if (!isPlaying) return;
    count++; currentCountDisplay.textContent = count;
    addTotalCount(1); updateProgress(); saveLiveCountToToday(1);
    const alertTrigger = parseInt(alertIntervalSelect.value);
    if (alertTrigger > 0 && count % alertTrigger === 0) triggerToastPopup();
    if (!isInfinite && count >= target) { finishSession(); return; }
    setTimeout(() => { if (isPlaying) { triggerBgSound(); nativeAudio.currentTime = 0; nativeAudio.play().catch(e => console.log(e)); } }, parseInt(speedSelect.value));
});

function updateProgress() { progressText.textContent = isInfinite ? `大悲修持中（無限循環）...` : `目標：${target} 次 / 已完成：${Math.round((count/target)*100)}%`; }

function triggerToastPopup() {
    alertMessage.textContent = alertQuotes[Math.floor(Math.random() * alertQuotes.length)];
    toastAlert.classList.add('show');
    if (toastTimer) clearTimeout(toastTimer);
    toastTimer = setTimeout(() => { toastAlert.classList.remove('show'); }, 5000);
}

startBtn.addEventListener('click', () => {
    if (!nativeAudio.src) return;
    initAudioContext(); isPlaying = true; count = 0;
    target = parseInt(targetCountSelect.value);
    currentCountDisplay.textContent = count; updateProgress();
    startBtn.disabled = true; stopBtn.disabled = false; recordBtn.disabled = false;
    triggerBgSound(); nativeAudio.currentTime = 0; nativeAudio.play().catch(err => stopAudio());
});

stopBtn.addEventListener('click', () => { stopAudio(); progressText.textContent = "修持手動暫停"; saveHistory(count, false); });

function stopAudio() { isPlaying = false; nativeAudio.pause(); startBtn.disabled = false; stopBtn.disabled = true; }

function finishSession() { stopAudio(); progressText.textContent = "🎉 功德圓滿，請及時迴向！"; saveHistory(count, true); if (navigator.vibrate) navigator.vibrate([200, 100, 200]); }
...
function saveHistory(finalCount, isCompleted) {
    ...
    history.unshift({ date: new Date().toLocaleString('zh-HK'), count: finalCount, status: isCompleted ? "功德圓滿" : "中途結束" });
...
function renderCalendar() {
    ...
    labels.forEach(l => { const div = document.createElement('div'); div.className = 'calendar-day-label'; div.textContent = l; calendarGrid.appendChild(div); });
    const firstDay = new Date(year, month, 1).getDay();
    const totalDays = new Date(year, month + 1, 0).getDate();
    for (let i = 0; i < firstDay; i++) { const emptyCell = document.createElement('div'); emptyCell.className = 'calendar-cell empty'; calendarGrid.appendChild(emptyCell); }
    const historyMap = JSON.parse(localStorage.getItem('zhunti_cal_history') || '{}');
    const todayStr = getTodayKey();
    for (let day = 1; day <= totalDays; day++) {
        const cell = document.createElement('div');
        cell.className = 'calendar-cell'; cell.textContent = day;
        const dateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
        const totalOnDay = historyMap[dateKey] || 0;
        if (totalOnDay > 0) {
            cell.classList.add('has-practice');
            const tooltip = document.createElement('span'); tooltip.className = 'tooltip'; tooltip.textContent = `已精進持誦 ${totalOnDay} 遍`; cell.appendChild(tooltip);
            if (dateKey === todayStr) cell.classList.add('today-practiced');
        }
        calendarGrid.appendChild(cell);
    }
}

prevMonthBtn.addEventListener('click', () => { currentNavDate.setMonth(currentNavDate.getMonth() - 1); renderCalendar(); });
nextMonthBtn.addEventListener('click', () => { currentNavDate.setMonth(currentNavDate.getMonth() + 1); renderCalendar(); });
clearHistoryBtn.addEventListener('click', () => {
    if(confirm('確定要清除所有每日持誦打卡紀錄？')) {
...
updateAllDedications(); renderCalendar(); renderTotalStats(); loadSavedAudio();
